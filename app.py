...
def iterative_panorama_generation() -> (Optional[Image.Image], Optional[Dict], Optional[float], str):
    ...
    #st.write("Starting iterative panorama generation...")
    max_iterations = 3
    best_match = None
    best_image = None
    best_analysis = None
    ...
    for iteration in range(max_iterations):
        ...
        while attempt < max_attempts:
            try:
                ...
                if result.get('status') == 'processing':
                    ...
                    #st.write(f"Generating panorama... Attempt {attempt + 1}/{max_attempts}")
                    poll_attempts = 0
                ...
                #st.write("Failed to generate/process image, retrying...")
            except Exception as e:
                #st.write(f"Error in generation attempt: {str(e)}")
                time.sleep(5)
            attempt += 1
            if attempt < max_attempts:
                #st.write(f"Retrying panorama generation in {current_delay} seconds...")
                time.sleep(current_delay)
                current_delay = min(current_delay * 1.5, max_delay)

        if image is None:
            #st.write("Failed to generate image in this iteration. Continuing to next iteration.")
            continue
        ...
        #st.write("Iteration analysis:")
        #st.write(f"Matched Elements: {len(matched_elements)}")
        #st.write(f"Missing Elements: {len(missing_elements)}")
        #st.write(f"Match Quality: {match_quality*100:.2f}%")
        ...
        if best_match is None or match_quality > best_match:
            ...
            #st.write("★ New best match achieved!")

        if match_quality >= 0.85:
            #st.write("Target match quality reached!")
            break

        if iteration < max_iterations - 1:
            current_prompt = improve_panorama_prompt(current_prompt, evaluation)
            #st.write("Prompt improved for next iteration.")
    ...
    if best_image and best_analysis:
        ...
        #st.write("No successful panorama was generated.")
...
with col1:
    ...
    # Always display the current panorama if it exists.
